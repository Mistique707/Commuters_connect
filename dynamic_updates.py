...
def publish_ride():
    ...
    try:
        ...
        return jsonify({
            "error":
            "Invalid input. Please check that all required fields are provided and formatted correctly."
        }), 400

    new_ride = Ride(user_email=session["user_email"],
                    departure=departure,
...
def get_rides():
    ...
    travel_date_str = request.args.get(
        'travel_date')  # Expected format: DD/MM/YYYY
    ...
    if travel_date_str:
        try:
            ...
            return jsonify(
                {"error": "Invalid date format. Expected DD/MM/YYYY."}), 400
    if gender:
...
def get_matches():
    ...
    try:
        user_ride = Ride.query.filter_by(
            id=int(ride_id), user_email=session["user_email"]).first()
        if not user_ride:
            return jsonify(
                {"error":
                 "Ride not found or not owned by the current user."}), 404
    except ValueError:
        return jsonify({"error": "Invalid ride_id parameter."}), 400

    candidates = Ride.query.filter(
        Ride.id != user_ride.id, Ride.travel_date == user_ride.travel_date,
        Ride.destination.ilike(f'%{user_ride.destination}%')).all()
...
def passengers_page():
    rides = Ride.query.all()
    return render_template('passengers.html', rides=rides)
